...
+ `热启动` 用户把 App 进程杀死，系统中存在 App 进程的缓存信息，用户点击 App，系统利用缓存信息给 App 创建进程启动
...
##### 0.2.1 +load 和 +initialize

+ `+load` 的调用先于main函数，用于注册一些非懒加载objc类的信息； `+initialize` 在main函数之后调用，在懒加载类型第一次出现时调用
+ 类目的 `+load` 分别存放在两个数组，不会覆盖；类目的 `+initialize` 会覆盖
+ `+load` 方法的调用是根据类继承关系调用，子类如果未实现则不会调用父类的 `+load` ； `+initialize` 是通过消息转发机制调用，子类未实现则进入父类的方法查找调用
...
+ 注册监听器和删除监听器必须成套出现，重复删除监听器会发生异常
+ keyPath为字符串类型，没有编译期检查
+ Swift中使用KVO需要继承NSObject
...
+ 规范化带来了实现上的复杂，必须遵守协议并实现指定的方法
+ 只能实现一对一的通信，如果多个对象都委托同一代理，为了区分不同的被委托对象，会造成业务的复杂化

### 5 数据持久化

#### 5.1 沙盒机制

iOS每个App都有自己的存储空间，这个存储空间叫做 `沙盒` ， App可以在自己的沙盒中进行数据存取操作，但不能访问其他App的沙盒空间。
对App做一些数据存储或者文件缓存时，一般都保存在沙盒中。

沙盒中的文件目录如下所示：

+ `Documents` 用来存储持久化数据的文件夹，程序运行时所需要的必要的文件都存储在这里，iCloud会自动备份
+ `Library` 一个文件目录，底下都是程序运行期间的设置和状态信息
  + `/Caches` 存放程序产生的可重新生成的数据，例如缓存文件、下载的内容等，系统发出磁盘低空间警告时，会清理该文件夹
  + `/Preferences` 用于存放应用程序偏好设置相关的文件，通常以 `.plist` 文件的形式存在，一般通过 `UserDefaults` 进行管理
  + 其他文件夹及自定义文件，用于存放特定数据
+ `tmp` 存放程序运行期间产生的临时文件，程序退出时会清理

#### 5.2 持久化方案

##### 5.2.1 属性列表Property List

即 `.plist` 文件，本质上是 `XML` ，可以存储一些轻量化的数据，数据量变大时会有性能问题，不支持自定义对象。

##### 5.2.2 偏好设置UserDefaults

本质上还是 `.plist` 文件，使用单例模式，便于编辑。
用于存储用户的偏好设置和用户信息，如用户名、是否自动登录等，数据保存在沙盒的 `Libarary/Preferences` 目录下。

##### 5.2.3 归档NSKeyedArchiver

可以将对象进行序列化和反序列化，需要遵守 `NSCoding` 协议。
归档后的文件是加密的，需要使用对应的解归档才能获取数据。由于加密的特性，因此需要额外的开销。

##### 5.2.4 JSON

可以通过 `NSJSONSerialization` 将对象序列化和反序列化为 `JSON` 文件，但对对象成员类型有限制要求：
...
在Swift中，可以通过 `Codable` 协议来自定义序列化和反序列化过程，实现自定义对象和JSON之间的转换。

##### 5.2.5 SQLite

`SQLite` 是目前主流的嵌入式关系型数据库，其最主要的特点就是轻量级、跨平台，可以用来存放规模稍大的数据，但由于是用C语言编写，因此一般不会直接使用，而是使用其封装版本 `FMDB` 或 `CoreData` 。

| - | FMDB | CoreData |
| :------: | :------- | :------- |
| 封装层次 | 用OC对SQLite进行了简单封装<br>满足了面向对象的要求 | 封装程度高，层次复杂 |
...
| 工具 | CocoaPods | Carthage | Swift Package Manager(SPM) |
| :------: | :------- | :------- | :------- |
| 适用语言 | OC/Swift | OC/Swift | OC/Swift/C/C++ |
| 侵入式 | 是 | 否 | 否 |
| 库数量 | 多 | 少于CocoaPods | 少于CocoaPods |
| 仓库 | 中心化 | 去中心化 | 去中心化 |
| 源码 | 可见 | 不可见 | 可见 |
| 编译速度 | 慢 | 快 | 首次慢 |
| 原理 | 将依赖库放入Pods项目中，<br>再让主项目依赖Pods | 将依赖库编译为动态库 | 自动执行依赖库下载、编译和链接 |
